(function(w) {
	var app = {
		...
		"openInstanceLogModal" : function(attemptId) {
			app.clearAlertInModal('instanceLogModal');
			// 拼接下载链接
			$("#instanceLogModal a[logLink]").attr("href",w.contextPath + "/task/instance/attemptlog/download?attemptId="+attemptId);
			// log
			$("#instanceLogModal pre[log]").text("");
			var uuid = app.GenerateUUID();
			$("#instanceLogModal pre[log]").attr("uuid",uuid);
			app.getAttemptLogTask(uuid,attemptId);
			$('#instanceLogModal').modal('show');
			// 自动刷新log
			app["autoGetLog_" + uuid] = true;
		},
		"getAttemptLogTask" : function(uuid, attemptId) {
			var param = new Object();
			param.attemptId = attemptId;
			param.uuid = uuid;
			$.ajax({
				type : "get",
				url : w.contextPath + "/task/instance/attemptlog/tail",
				data : param,
				dataType : "json",
				success : app.getAttemptLogTaskDone(uuid, attemptId),
				error : app.httpError
			});
		},
		"getAttemptLogTaskDone" : function(uuid, attemptId) {
			return function(data) {
				if (data.success == false) {
					app.alertErrorInModal('instanceLogModal',data.errorMsg);
//					$("#instanceLogModal div[alertMessage]").html(
//							$("#alert_error").html());
//					$("#instanceLogModal div[alertMessage] > div > span").text(
//							data.errorMsg);
				} else {
						var pre = $("#instanceLogModal pre[log]");
						var text = pre.text();
						if (text == null || text != data.log) {
							pre.text(text + data.log);
						}
						if(app["autoGetLog_" + uuid] && data.continue){
							app.getAttemptLogTask(uuid,attemptId);
						}
				}
			}
		},
		"clearAttemptLogCache" : function(uuid){
			var param = new Object();
			param.uuid = uuid;
			$.ajax({
				type : "get",
				url : w.contextPath + "/task/instance/attemptlog/clearCache",
				data : param
			});
		},
		// ///////////////////////////////////////////////////////////////////
		...
		"clearAlertInModal" : function(modalId) {
			$("#" + modalId + " div[alertMessage]").html('');
		},
		"alertErrorInModal" : function(modalId, msg) {
			var div = $("#" + modalId + " div[alertMessage]");
			div.html($("#alert_error").html());
			div.find("div > span").text(msg);
		},
		"alertSuccessInModal" : function(modalId, msg) {
			var div = $("#" + modalId + " div[alertMessage]");
			div.html($("#alert_success").html());
			div.find("div > span").text(msg);
		},
		"alertWarnInModal" : function(modalId, msg) {
			var div = $("#" + modalId + " div[alertMessage]");
			div.html($("#alert_warn").html());
			div.find("div > span").text(msg);
		},
		...
		/**
		 * 生成UUID app.GenerateUUID()--->5BC35817-4FE8-4E94-B922-DB3D5E5E5F0E
		 * app.GenerateUUID(len)--->生成len位的UUID app.GenerateUUID(len,
		 * radix)--->生成len位且是radix进制的UUID，如GenerateUUID(10,
		 * 2)生成1010001010、GenerateUUID(10, 16)生成C15FC4A930
		 */
...
$(document).ready(function() {
	...
	// 关闭log modal时，发出清理（尽管只有2个页面，但全范围加也无大碍）
	var clearAttemptLogCache = function() {
	  	  var uuid=$("#instanceLogModal pre[log]").attr("uuid");
	  	  if(uuid){
	  		  $("#instanceLogModal pre[log]").attr("uuid","");
	  		  app.clearAttemptLogCache(uuid);
	  		  app["autoGetLog_" + uuid] = false;
	  	  }
		};
	$('#instanceLogModal').on('hidden.bs.modal', clearAttemptLogCache);
	window.onunload = clearAttemptLogCache;
});
