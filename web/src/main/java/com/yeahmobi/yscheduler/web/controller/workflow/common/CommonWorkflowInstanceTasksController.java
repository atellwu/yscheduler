 ...
 */
@Controller
@RequestMapping(value = { CommonWorkflowInstanceTasksController.SCREEN_NAME })
public class CommonWorkflowInstanceTasksController extends AbstractController {

    public static final String      SCREEN_NAME = "common/instance/tasks";
    ...
    public ModelAndView index(Integer pageNum, long instanceId) {
        ...
        if (UserContextHolder.getUserContext().isAdmin()) {
            taskInstances = this.taskInstanceService.listByWorkflowInstanceId(instanceId, pageNum, paginator);
        } else {
            taskInstances = this.taskInstanceService.listByWorkflowInstanceIdAndUserId(instanceId,
