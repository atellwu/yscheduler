...
public class CalloutTaskExecutor<T extends AgentTask> extends BaseTaskExecutor<T> {
    ...
    private void callout(TaskTransaction taskTransaction, Map<String, String> params) throws Exception {
        ...
        String workflowName = params.get(AbstractAgentTask.PARAM_WORKFLOW_NAME);
        if (workflowName != null) {
            this.calloutParams.put(AbstractAgentTask.PARAM_WORKFLOW_NAME, workflowName);
        }
        String taskName = params.get(AbstractAgentTask.PARAM_TASKNAME);
        if (taskName != null) {
            this.calloutParams.put(AbstractAgentTask.PARAM_TASKNAME, taskName);
        }
    ...
    private void validate(Map<String, String> params) throws IllegalArgumentException {
        // callout
        String calloutUrl = params.get(CalloutAgentTask.PARAM_CALLOUT_URL);
        ...
        this.needCallback = BooleanUtils.toBoolean(params.get(CalloutAgentTask.PARAM_NEED_CALLBACK));
        this.agentHost = params.get(AbstractAgentTask.PARAM_AGENT_HOST);
        if (this.needCallback) {
            Validate.notEmpty(this.agentHost, "agentHost cannot be empty since needCallback");
        }
        // cancel
        String cancelUrl = params.get(CalloutAgentTask.PARAM_CANCEL_URL);
    ...
    private boolean needCallback(Map<String, String> params) {
        return BooleanUtils.toBoolean(params.get(CalloutAgentTask.PARAM_NEED_CALLBACK));
    }
